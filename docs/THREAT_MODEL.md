...
The implemented bundle is hash-verified and tamper-evident. SHA-256 values can show that a file differs from the value recorded in the manifest. They do not prove who controlled the host, provide trusted time, or prevent coordinated replacement of both an artifact and its manifest entry. The evidence document does not contain its own hash because that would create a circular definition.
